...
def wid(players, playerNamesList, im_dir):
	...
	answer = [K_r, K_f] # 12: correct, 13: wrong
	...
	#numberPlayers = len(playerScore)
	print "number of players:", players
	
	# set the screen the highest resolution for this display
	screen=pygame.display.set_mode((1200,800), pygame.FULLSCREEN)#(1800, 860))#
	...
	welcome = "Willkommen bei 'Wer ist das?'"
	...
	def random_pick_image():
		...
		try:    
	    		...
	    		random_im_key = "Alle Bilder gespielt! Ende"
		...
		if not bool(image_dict) and random_im_val == logo:
		    solution_label = myfont.render("Alle Bilder gespielt! Ende!", 1, red)
